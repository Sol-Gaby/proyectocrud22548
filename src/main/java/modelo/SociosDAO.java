...
public class SociosDAO {
    ...
    public Socios mostrarSocio(int _id) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String direccion = rs.getString("direccion");
                String localidad = rs.getString("Localidad");
                LocalDate fnac = rs.getDate("fechaNac").toLocalDate();
                String email = rs.getString("email");
                String telefono = rs.getString("telefono");
                boolean activo = rs.getBoolean("activo");
    ...
    public boolean InsertSocios(Socios s1) {
        ...
        try {
            ps = conexion.prepareStatement("insert into socios (nombre,apellido,direccion,localidad,fechaNac,email,telefono,activo) values (?,?,?,?,?,?,?,?)");
